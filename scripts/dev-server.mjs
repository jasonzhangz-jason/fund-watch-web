...
// 根 index.html 现为 Vite 前端入口（需打包器，本服务器无法直接渲染），
// 因此本服务器在 / 上提供归档版单页 UI（legacy/），新版 React UI 由 `pnpm dev`(vite:5173) 提供。
const LEGACY_UI = existsSync(path.join(ROOT, 'legacy', 'index.html')) ? 'legacy/index.html' : 'index.html';
...
const handlers = {
  ...
  holdings: require('../api/holdings.js'),
};
...
async function routeAccount(req, url, query, cookies, body) {
  ...
  // 持仓（账号级持久化）
  if (p === '/api/positions' && req.method === 'GET') return accountHandlers.listPositions({ cookies });
  if (p === '/api/positions' && req.method === 'POST') return accountHandlers.savePosition({ cookies, body });
  if (p === '/api/positions' && req.method === 'DELETE') {
    return accountHandlers.removePositions({ cookies, body, query });
  }
  if (p === '/api/positions/reorder' && req.method === 'POST') {
    return accountHandlers.reorderPosition({ cookies, body });
  }
  if (p === '/api/watchlist/reorder' && req.method === 'POST') {
    return accountHandlers.reorderWatchlist({ cookies, body });
  }

  // 账本聚合（账户资产 / 当日收益 / 每日快照）与行情缓存
...
<span style="opacity:.7">页面地址：/__qr?url=http://自定义地址</span></div>
  ...
  // ---- 账号 / 自选 / 持仓 / 后台（SQLite 持久化）----
  if (
    url.startsWith('/api/auth/') ||
    url === '/api/watchlist' ||
    url.startsWith('/api/watchlist/') ||
    url === '/api/positions' ||
    url.startsWith('/api/positions/') ||
    url === '/api/portfolio' ||
  ...
  // ---- 静态文件 ----
  const file = path.join(ROOT, url === '/' ? LEGACY_UI : url);
  try {
...
server.listen(PORT, () => {
  ...
  console.log('');
  console.log('┌──────────────────────────────────────────────────────────┐');
  console.log(`│  📈 fund-watch-web 后端调试服务器                         │`);
  console.log(`│  本机   http://localhost:${PORT}                            │`);
  for (const u of lan) console.log(`│  局域网 ${u}${' '.repeat(Math.max(1, 17 - u.length))}│`);
  console.log(`│  界面   归档版单页 UI（新版 React UI: pnpm dev → 5173）    │`);
