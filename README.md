...
| `/correlation` | **基金相关性分析** | 新增 | 账本内基金的**走势相似程度**：平均相关性 + 解读、**相关性矩阵（热力图）**、最相似 / 最分散组合、两两组合明细（带等级标签），支持近 30/60/120 交易日切换 |
...
pnpm test:correlation   # 基金相关性分析：结构 + **独立复算皮尔逊系数逐值比对** + 窗口/缓存：32 项断言
...
| **基金相关性分析** | 新增 `GET /api/portfolio/correlation` + 页面 `/correlation`：共同交易日对齐后对**日收益率**求皮尔逊系数，返回 N×N 矩阵（热力图）、两两组合（含等级）、最相似/最分散与平均相关性解读；净值历史支持自动翻页（上游每页限 20 条）并缓存 30 分钟 |
