...
| 路由 | 页面 | 对应提示词 | 数据来源与交互 |
|---|---|---|---|
| `/` | 账本主页 | `主页-1.md` | 持仓来自账号；**涨跌幅与当日收益接真实行情**（当日收益 = 金额×涨幅，估算）；表头排序、⊕ 操作浮层、**点击持仓行进入该基金详情页** |
...
- **自适应**：手机宽度全屏铺满；桌面端自动套 393×852 手机外框（顶部保留真实设备安全区 `safe-area-inset-top`）
...
| 系统运营数据 | 10 个指标卡，按 **账号 / 自选 / 账本 / 数据** 四组着色：注册用户、管理员、活跃会话、自选条数、有自选用户、持仓条数、有持仓用户、行情缓存、快照天数（含最新快照日期）、每日明细行；`60s 自动刷新` + 手动刷新 |
| 用户列表 | `id / 用户名 / 角色 / 自选数 / 活跃会话 / 注册时间`，支持按用户名搜索（`/api/admin/users`）；**PC/平板为表格，手机自动切换为卡片列表** |
...
> 删除用户、重置密码等写操作接口（`DELETE /api/admin/users/:id`、`POST` 重置密码）后端已就绪，后台页当前只做**只读展示**。
| 数据源切换 | 同上（estimate / nav） | 「盘中估值 / 最新净值 / 累计净值」三项**各对应真实字段**，切换即改变详情页展示口径 |
...
pnpm test:admin         # 后台权限与用户管理 + 运营统计字段：32 项断言
...
pnpm test:live          # 未登录无静态数据 → 注册 → 落库 → 下拉/60s 刷新 → 行点击跳详情 → 我的 → 重仓股 → 管理员后台 + 后台页三档自适应 → 重启仍在：78 项断言
...
| 接口 | 读写 | 说明 |
|---|---|---|
| `GET /api/portfolio` | 读 `positions` + `fund_quotes`，写 `position_daily` + `account_daily` | **账户账本**：账户资产、持有收益、当日总收益、持仓明细（含每只净值/涨幅/当日收益）；`?refresh=1` 强制刷新上游行情 |
...
| `GET /api/admin/*` | 读 `users` / `sessions` / `watchlist` | 后台统计与用户管理（按 `users.role` 鉴权） |
